...
resource "aws_security_group" "ecs_api" {
  ...
  egress {
    description = "Allow HTTPS to internet (for AWS services, MongoDB Atlas)"
    from_port   = 443
    to_port     = 443
    protocol    = "tcp"
    cidr_blocks = ["0.0.0.0/0"]
  ...
  egress {
    description = "Allow MongoDB Atlas default port"
    from_port   = 27017
    to_port     = 27017
    protocol    = "tcp"
    cidr_blocks = ["0.0.0.0/0"]
...
resource "aws_security_group" "ecs_ui" {
  ...
  egress {
    description = "Allow HTTPS to internet (for AWS services)"
    from_port   = 443
    to_port     = 443
    protocol    = "tcp"
    cidr_blocks = ["0.0.0.0/0"]
...
resource "aws_iam_role" "ecs_task_api" {
  name = "${var.project_name}-${var.environment}-ecs-task-api-role"
...
# ECS Task Role for UI
resource "aws_iam_role" "ecs_task_ui" {
  name = "${var.project_name}-${var.environment}-ecs-task-ui-role"
